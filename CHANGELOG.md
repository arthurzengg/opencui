...
- Intermediate assistant messages (`processOnly` mode — when a Hephaestus-style turn emits multiple sub-task messages) no longer show their per-step `model · tokens` usage line. The collapsed step panel and the final-message usage were rendering side-by-side, making completed steps look identical to the still-running step. Usage now only renders on the final message of a turn.
)" 
