...
- Toast dedup no longer breaks on spinner-frame animations. Some agents (OhMyOpenCode etc.) animate a spinner by sending the same toast every ~100 ms with a different leading glyph (`·`, `•`, `●`, `○`, `◌`, `◦`, …). The dedup key now normalizes leading punctuation/symbols and whitespace before comparing, so all frames collapse into one logical toast.
