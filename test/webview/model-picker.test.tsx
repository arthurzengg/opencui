...
describe("ModelPicker", () => {
  ...
  it("renders the chip rows as a footer: list first, then Agent, then Effort", () => {
    ...
    expect(list.compareDocumentPosition(agent) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
    expect(agent.compareDocumentPosition(effort) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy()
    // Each row is its own bordered block (the divider CSS keys on these), so
    // the list/Agent and Agent/Effort seams draw the same line.
    expect(agent.parentElement!.className).toBe("model-picker-agents")
    expect(effort.parentElement!.className).toBe("model-picker-effort")
  })
